...
const extractedFields = [
  ['Primary target / suspect', 'subject', 'Ramesh Kumar (alias: “Wire”)'],
  ['FIR registry identifier', 'firId', 'FIR-CR-9402/2026-NZ'],
  ['Incident occurrence timestamp', 'incidentAt', '2026-03-18 • 02:40 IST'],
  ['Linked identity UID / KYC ref', 'identityRef', 'KYC-UID-8941-2094-1182'],
  ['Primary incident location', 'location', 'Warehouse Bay 4, Pier 44 Maritime Freight Terminal'],
]

export function CaseIntakeWizard({ onComplete, onExit }) {
  ...
  const [form, setForm] = useState({
    ...
    priority: 'High',
    firId: 'FIR-CR-9402/2026-NZ',
    incidentAt: '2026-03-18 • 02:40 IST',
    identityRef: 'KYC-UID-8941-2094-1182',
    location: 'Warehouse Bay 4, Pier 44 Maritime Freight Terminal',
  })
  ...
  return <div className="min-h-screen bg-[#f5f1ea] text-[#171511]">
    <header className="sticky top-0 z-20 border-b border-[#e5e0d8] bg-[#f5f1ea]/90 backdrop-blur">
      <div className="mx-auto flex h-16 max-w-5xl items-center justify-between px-4 sm:px-8">
        <div className="flex items-center gap-3"><span className="grid size-7 place-items-center rounded bg-[#111] text-white"><Sparkles className="size-4" /></span><p className="text-[13px] font-semibold uppercase tracking-wider">Nexus // Intel</p><span className="hidden rounded-full bg-[#ece8e1] px-3 py-1 font-mono text-[10px] uppercase tracking-wider text-[#666052] sm:inline">Case initiation // wizard</span></div>
    ...
    <main className="mx-auto max-w-5xl px-4 py-8 sm:px-8 sm:py-12">
      <Progress step={step} />
      <section className="animate-rise-in mb-7"><h1 className="font-serif text-[30px] leading-tight sm:text-[36px]">{step === 1 ? <>Start a new <em>investigation</em></> : step === 2 ? <>Upload the <em>dossier</em></> : step === 3 ? <>Verify extracted <em>intelligence</em></> : <>Confirm & <em>link</em> the case</>}</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-[#777166]">{step === 1 ? 'Create the case record before evidence enters the intelligence ledger.' : step === 2 ? 'Attach an FIR or supporting evidence file for extraction and review.' : step === 3 ? 'Cross-reference automated annotations before they are added to the master graph.' : 'Review the case package before publishing it to the investigation graph.'}</p></section>
...
function CaseInfo({ form, update }) {
  return <div><SectionTitle icon={FileText} title="Case information" text="Add the core details used to open the investigation record." /><div className="grid gap-5 sm:grid-cols-2"><Input label="Case title" value={form.title} onChange={(value) => update('title', value)} /><Input label="Primary target / subject" value={form.subject} onChange={(value) => update('subject', value)} /><Input label="Case identifier" value={form.caseId} mono onChange={(value) => update('caseId', value)} /><label className="block"><span className="mb-2 block text-xs font-semibold">Priority</span><select value={form.priority} onChange={(event) => update('priority', event.target.value)} className="h-11 w-full rounded-lg bg-[#faf7f2] px-3 text-sm outline-none transition focus:bg-white focus:ring-1 focus:ring-[#171511]"><option>High</option><option>Medium</option><option>Low</option></select></label></div></div>
}

function UploadDossier({ dossier, setDossier }) {
  return <div><SectionTitle icon={FolderUp} title="Evidence dossier" text="Attach a PDF, image, or text record. It will be shown for review in the next step." /><label className="flex min-h-56 cursor-pointer flex-col items-center justify-center rounded-xl border border-dashed border-[#c9c2b5] bg-[#faf7f2] p-6 text-center transition hover:border-[#171511] hover:bg-[#f5f1ea]"><Upload className="size-8 text-[#846019]" /><p className="mt-4 text-sm font-semibold">{dossier ? dossier.name : 'Choose a dossier file'}</p><p className="mt-1 text-xs text-[#777166]">PDF, image, or text document · files remain local in this demo</p><input className="sr-only" type="file" accept=".pdf,image/*,.txt" onChange={(event) => setDossier(event.target.files?.[0] || null)} /></label>{dossier ? <p className="mt-4 flex items-center gap-2 rounded-lg bg-[#f4ead8] p-3 text-xs text-[#6e4f14]"><ShieldCheck className="size-4" /> {dossier.name} is ready for extraction.</p> : <p className="mt-4 text-xs text-[#777166]">No file selected. You can still continue with the sample extraction data.</p>}</div>
}

function ReviewExtracted({ form, update }) {
  return <div><SectionTitle icon={Sparkles} title="Extracted entities & metadata" text="Review the proposed values and correct any uncertainty before publishing." /><div className="space-y-4">{extractedFields.map(([label, key, fallback], index) => <label key={key} className="block rounded-lg bg-[#faf7f2] p-4"><span className="mb-2 flex items-center justify-between gap-3 text-xs font-semibold"><span>{label}</span><span className="rounded-full bg-white px-2 py-1 text-[10px] font-medium text-[#777166]"><Sparkles className="mr-1 inline size-3 text-[#c08a2e]" />{98 - index * 2}% confidence</span></span><input value={form[key] || fallback} onChange={(event) => update(key, event.target.value)} className="h-10 w-full rounded-lg bg-white px-3 text-sm outline-none ring-1 ring-[#e5e0d8] transition focus:ring-[#171511]" /></label>)}</div></div>
}

function ConfirmLink({ form, dossier }) {
  return <div><SectionTitle icon={ShieldCheck} title="Ready to link" text="Confirm the final case package before it becomes part of the intelligence graph." /><div className="grid gap-3 sm:grid-cols-2"><Summary label="Case" value={`${form.caseId} · ${form.title}`} /><Summary label="Primary subject" value={form.subject} /><Summary label="Priority" value={`${form.priority} risk`} /><Summary label="Evidence" value={dossier?.name || 'Sample FIR extraction'} /></div><p className="mt-5 flex gap-2 rounded-lg bg-[#faf7f2] p-4 text-xs leading-5 text-[#666052]"><ShieldCheck className="size-4 shrink-0 text-[#846019]" />Linking records this case, its supporting evidence reference, and verified entities in the audit log.</p></div>
}
